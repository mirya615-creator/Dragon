...
| `ad_request`, `ad_result` | Optional `IAdAnalyticsObserverV2` injected into the ad service wrapper. | Default is NoOp, so Mock ads do not pollute Firebase. Ad completion does not imply reward; server verification must produce the authoritative grant plus `ledger_result`. | Client observation seam reserved; production SDK and backend reward verification pending. |
